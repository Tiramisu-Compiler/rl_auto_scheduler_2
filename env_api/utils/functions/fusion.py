    ...
    for fusion in fusions:
        ...
        if len(branch_comp_0) >= len(branch_comp_1):
            ...
            for i in range(level, 0, -1):
                if len(branch_comp_1[i]['computations_list']) == 0 and len(branch_comp_1[i]['child_list']) == 0:
            ...
            # Remove the empty nodes from new_tree
            for i in range(level, 0, -1):
                if len(branch_comp_0[i]['computations_list']) == 0 and len(branch_comp_0[i]['child_list']) == 0:
