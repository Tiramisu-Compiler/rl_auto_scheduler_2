...
class Model_Recursive_LSTM_Legality(nn.Module):
    def __init__(
        self,
        input_size,
    ...
    def get_hidden_state(self, node, comps_embeddings, loops_tensor):
        ...
        selected_loop_tensor = torch.index_select(
            loops_tensor, 1, node["loop_index"].to(self.device)
        )
    ...
    def forward(self, tree_tensors):
        ...
        batch_size, num_comps, len_sequence, len_vector = (
            functions_comps_expr_tree.shape
        )

        # Expressions embedding layer
        x = functions_comps_expr_tree.view(
            batch_size * num_comps, len_sequence, len_vector
        )
        ...
        first_part = comps_tensor_first_part.to(self.device).view(
            batch_size * num_comps, -1
        )
        vectors = comps_tensor_vectors.to(
            self.device
        )  # No need to reshape this tensor since we transformed it when loading the data
        third_part = comps_tensor_third_part.to(self.device).view(
            batch_size * num_comps, -1
        )
        ...
        for root in tree["roots"]:
            ...
            roots_list.append(
                self.get_hidden_state(root, comps_embeddings, loops_tensor)
            )
