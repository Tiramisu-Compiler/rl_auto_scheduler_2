...
class PredictionService:
    ...
    def get_real_speedup(self, schedule_object : Schedule,branches : List[Branch]):
        ...
        if "initial_execution" in schedule_object.prog.execution_times :
            ...
            initial_execution = schedule_object.prog.execution_times["initial_execution"]
        else : 
            # We need to run the program to get the value 
            initial_execution = CompilingService.execute_code(tiramisu_program=schedule_object.prog,
                                                            optims_list=[],branches=branches)
            if initial_execution : 
                schedule_object.prog.execution_times["initial_execution"] = initial_execution
            else : 
                raise ExecutingFunctionException
        
        if schedule_object.schedule_str in schedule_object.prog.execution_times :

            schedule_execution = schedule_object.prog.execution_times[schedule_object.schedule_str]

        else : 
            # We need to run the program to get the value 
            schedule_execution = CompilingService.execute_code(tiramisu_program=schedule_object.prog,
                                                            optims_list=schedule_object.schedule_list,
                                                            branches=branches)
            if schedule_execution : 
                schedule_object.prog.execution_times[schedule_object.schedule_str] = schedule_execution
            else : 
                raise ExecutingFunctionException   
