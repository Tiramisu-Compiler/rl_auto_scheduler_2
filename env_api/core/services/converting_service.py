...
class ConvertService: 
    @classmethod
    def get_computations_representation(cls, program_annot):
        max_depth = 5
        max_accesses = 21
        ...
        for index, comp_name in enumerate(ordered_comp_list):
            ...
            # iterators_repr Looks like this : 
            # [1, 769, 'Li0Interchanged', 'Li0Skewed', 'Li0SkewFactor', 'Li0Parallelized', 'Li0Tiled', 'Li0TileFactor', 'Li0Reversed', 'Li0Fused', 0, 0, 'Li0_1Interchanged', 'Li0_1Skewed', 'Li0_1SkewFactor', 'Li0_1Parallelized', 'Li0_1Tiled', 'Li0_1TileFactor', 'Li0_1Reversed', 'Li0_1Fused', 0, 768, 'Li1Interchanged', 'Li1Skewed', 'Li1SkewFactor','Li1Parallelized', 'Li1Tiled', 'Li1TileFactor', 'Li1Reversed', 'Li1Fused', 0, 0, 'Li1_1Interchanged', 'Li1_1Skewed', 'Li1_1SkewFactor', 'Li1_1Parallelized', 'Li1_1Tiled', 'Li1_1TileFactor', 'Li1_1Reversed', 'Li1_1Fused']
            ...
            # Size of iterators_repr will become 102

            
            comp_representation.extend(iterators_repr)
            # print('***********--------------------')
            # print(comp_representation)
            # print('***********--------------------')           
            ...
            # Size of comp_representation will become 145
            ...
            # Size of comp_representation will become 1048  
            ...
            # Zero all features
            # program_representation.append(comp_representation[:102]+[0]*(950))
    ...
    @classmethod
    def get_schedule_template(cls,annotations, sched_dict, max_depth,max_matrices = 5):
        max_accesses = 15
        min_accesses = 1
        #     max_depth = 5
        ...
        for comp_index, comp_name in enumerate(ordered_comp_list):
            ...
            iterators_repr = []
            for iter_i, iterator_name in enumerate(comp_dict["iterators"]):
                iterator_dict = annotations["iterators"][iterator_name]
                iterators_repr.extend(
                    [iterator_dict["lower_bound"], iterator_dict["upper_bound"]]
                )
        ...
        #     assert len(annotations['iterators'])==len(set(annotations['iterators'])) #just to make sure that loop names are not duplicates, but this can't happen because it's a dict
        ...
        for loop_index, loop_name in enumerate(annotations["iterators"]):
            loop_repr_template = []
            l_code = "L" + loop_name
            # upper and lower bound
            loop_repr_template.extend(
                [
                    annotations["iterators"][loop_name]["lower_bound"],
                    annotations["iterators"][loop_name]["upper_bound"],
                ]
            )
            loop_repr_template.extend(
                [
        ...
        if "transformation_matrices" in comp_schedule_dict:
            ...
            if comp_schedule_dict["skewing"]:
                ...
                if (first_factor, second_factor) in global_dioph_sols_dict:
                    ...
                    a, b = cls.linear_diophantine_default(first_factor, second_factor)
