import copy,re, torch , numpy as np
...
class ConvertService: 
    ...
    def get_representation_template(cls, program_json, no_sched_json, max_depth = MAX_DEPTH, train_device="cpu"):
        ...
        for comp_index, comp_name in enumerate(ordered_comp_list):
            ...
            comps_expr_repr_templates_list.append(cls.get_tree_expr_repr(expr_dict, comp_type))
        ...
        prog_tree = cls.update_tree_atributes(tree_annotation, loops_indices_dict, comps_indices_dict, train_device=train_device)
    ...
    @classmethod
    def update_tree_atributes(cls,node, loops_indices_dict, comps_indices_dict, train_device="cpu"):
            if "roots" in node :
    ...
    def format_bound(cls,iterator_name, bound, iterators_list, is_lower):
        ...
        return output
    @classmethod
    def get_padded_initial_constrain_matrix(cls,program_json, schedule_json, comp_name, max_depth):
        iterators_list = program_json["computations"][comp_name]["iterators"]
        transformation_matrix = cls.get_transformation_matrix(program_json, schedule_json, comp_name, max_depth)
        result = []
        for i in iterators_list:
            for j in range(2):
                if j == 0 :
                    result.append(cls.format_bound(i, program_json["iterators"][i]["lower_bound"], iterators_list, True))
                else:
                    result.append(cls.format_bound(i, program_json["iterators"][i]["upper_bound"], iterators_list, False))
        ...
        return result
    @classmethod
    def get_padded_transformed_constrain_matrix(cls,program_json, schedule_json, comp_name, max_depth):
        iterators_list = program_json["computations"][comp_name]["iterators"]
        transformation_matrix = cls.get_transformation_matrix(program_json, schedule_json, comp_name, max_depth)
        result = []
        for i in iterators_list:
    ...
    def get_transformation_matrix(cls,program_json, schedule_json, comp_name, max_depth=None):
        ...
        for transformation in schedule_json[comp_name]["transformations_list"]:
            matrix = cls.get_trasnformation_matrix_from_vector(transformation, nb_iterators)
    ...
    # Create a copy of the templates to avoid modifying the values for other schedules
        ...
        for comp_index, comp_name in enumerate(ordered_comp_list):
            ...
            comps_repr[ogc_start[0]][ogc_start[1] : ogc_end[1] + 1 ] = cls.get_padded_initial_constrain_matrix(program_json, schedule_json, comp_name, max_depth_it).flatten().tolist()
            ...
            comps_repr[ogv_start[0]][ogv_start[1] : ogv_end[1] + 1 ] = cls.get_padded_second_side_of_the_constraint_equations_original(program_json, schedule_json, comp_name, max_depth)
            ...
            comps_repr[c_start[0]][ c_start[1] : c_end[1] + 1 ] = cls.get_padded_transformed_constrain_matrix(program_json, schedule_json, comp_name, max_depth_it).flatten().tolist()
    ...
    #             comp_innermost_loop = computations_dict[comp_name]["iterators"][-1]
        ...
        program_iterators = cls.get_comp_iterators_from_tree_struct(schedule_json, comp_name)
    ...
    @classmethod
    def get_padded_second_side_of_the_constraint_equations_original(cls,program_json, schedule_json, comp_name, max_depth):
        iterators_list = program_json["computations"][comp_name]["iterators"]
        result = []
    ...
    @classmethod
    def seperate_vector(
        cls,X: torch.Tensor, num_transformations: int = 4, pad: bool = True, pad_amount: int = 5
    ) -> torch.Tensor:
