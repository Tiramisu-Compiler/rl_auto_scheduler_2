...
class Model_Recursive_LSTM_v3(TorchModelV2,nn.Module):
    # TODO : give model params in model_config
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        ...
        # The last regression layers 
        # The shape of input is (1, 1, 180)
        ...
        if "roots" in node:
            ...
            if node["has_comps"]:
                selected_comps_tensor = torch.index_select(
                    comps_embeddings, 1, torch.tensor(node["computations_indices"]).to(
                        self.train_device)
                )
